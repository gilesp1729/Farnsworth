...
* 6-pin analog torque/cadence sensor in M510 or M560 motor (the CAN bus ones are not supported). Depending on the motor they have different counts of pulses per crank revolution.
...
Arduino Nano 33 BLE or BLE Sense. Any VESC should work if it has the standard inputs for UART and motor temp.
...
* 0.1uF caps for noise suppression on speed and cadence lines are not on board yet.

* The green connector is for the speed sensor (open collector two-wire Hall from Bosch)

* The torque sensor uses only 4 of the 6 wires: +5V, GND, SD and analog torque. SD is 3.3V logic, while the torque is divided down to keep voltages exceeding 3.3 out of the Arduino.
...
The lid is made in two halves so it can be slipped in under the shock. It's not waterproof but I won't be fording any creeks with it.
